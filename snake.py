...
class Snake:
    ...
    def reset(self):
        ...
        self.length = 3
        # Initialiser le serpent avec 3 positions align├®es dans la direction
        start_x = GRID_WIDTH // 2
        start_y = GRID_HEIGHT // 2
        self.positions = []
        for i in range(self.length):
            # Cr├®er les positions en reculant de la direction
    ...
    def move(self, food):
        ...
        # Ajouter la nouvelle position de la t├¬te
        self.positions.insert(0, new)

        if new == food:
            # Le serpent a mang├® : augmenter le score et la taille
            self.score += 1
            self.length += 1  # Augmente la taille du serpent
            self.steps_without_food = 0
            # Ne pas tronquer : la liste a maintenant self.length ├®l├®ments (ancienne longueur + 1 nouvelle position)
            return True

        # Si le serpent n'a pas mang├®, tronquer pour garder exactement self.length ├®l├®ments
        # On garde les self.length premi├¿res positions (la t├¬te + le corps)
